...
Antigravity Go 바이너리에는 **17개 브라우저 ToolConverter**가 존재한다. 현재 agent-browser에서 구현된 것과 비교하면 **10개 이상의 기능이 누락**되어 있다.

---

## 1. Antigravity 브라우저 도구 전체 목록 (17개)

| # | Tool | Tool ID | agent-browser 현재 | 상태 |
|---|------|---------|:---:|:---:|
...
agent-browser에는 이 수준의 상태 관리가 없다. 프로세스 격리 방식이므로 명령 간 상태를 유지하지 않는다.
...
| Property | 의미 | agent-browser 적용 가능? |
|----------|------|:---:|
