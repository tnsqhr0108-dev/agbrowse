...
| ChatGPT code mode (`web-ai code`) | beta | `web-ai/code-mode.mjs`, `web-ai/code-mode-prompt.mjs`, `web-ai/code-artifact.mjs` | `test/unit/web-ai-code-mode*.test.mjs`, `test/unit/web-ai-code-artifact.test.mjs` | n/a; agbrowse-only beta for strict contract prompt → headless single/multi zip retrieval via in-page presigned fetch. Not mirrored in cli-jaw. |
