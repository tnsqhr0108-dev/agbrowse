...
| Adaptive URL fetch (`agbrowse fetch <url>`) | experimental | `skills/browser/adaptive-fetch/*.mjs`, `skills/browser/browser.mjs` | `test/unit/browser-adaptive-fetch-*.test.mjs`, `test/integration/browser-fetch-command.test.mjs` | planned as `cli-jaw browser fetch <url>` after agbrowse v1 proves stable. URL reader only; not generic search. Public endpoint ladder includes GitHub/Reddit/HN/Wikipedia/npm/PyPI/arXiv plus Bluesky/Mastodon/Stack Exchange/dev.to/CrossRef/OpenLibrary/Wayback/YouTube/X oEmbed/HN Algolia/V2EX/Lobsters. |
