...
| ChatGPT code mode (`web-ai code`) | beta | `web-ai/code-mode.mjs`, `web-ai/code-mode-prompt.mjs`, `web-ai/code-artifact.mjs` | `test/unit/web-ai-code-mode*.test.mjs`, `test/unit/web-ai-code-artifact.test.mjs` | strict contract prompt → headless `/mnt/data/result.zip` retrieval (in-page presigned fetch). ChatGPT-only; not mirrored in cli-jaw. |
