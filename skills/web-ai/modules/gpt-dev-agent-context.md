...
- The plan file must include:
  ...
  - A top-level checklist ordered by the actual work plan. Use at most 8 visible/top-level checklist items.
  - If the work needs more than 8 steps, add a separate textual "Detailed stage instructions" section instead of creating more visible checklist items.
  - Final checkbox state: mark completed plan items with `[x]` before packaging the zip. Do not leave completed work as `[ ]`.
  - Implementation notes.
  - Verification commands attempted.
  - Packaging rules and excluded artifacts.
- If a visible todo tool such as `turn_plan.update_turn_plan` is available, use it before writing code with at most 8 top-level items. If the task needs more detail, keep the visible todo at 8 or fewer items and put the extra steps in the plan markdown as text. Update that visible checklist as implementation, verification, and packaging progress. The visible todo UI may disappear after the response finishes; do not rely on it as the durable artifact.
- If the visible todo tool is not available, do not pretend it was called; the plan markdown is the durable checklist.
- Before packaging, update the plan markdown so completed checklist items are marked `[x]`.
